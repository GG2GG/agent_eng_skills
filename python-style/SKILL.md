...
- Tokens live in **per-project `.env` files**, not a single global one. Never hardcode paths to other projects' `.env` files — each project owns its own credentials. Refer to the `Credential Hygiene` section in your project's CLAUDE.md for canonical token locations.
