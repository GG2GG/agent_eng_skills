...
- **2026-05-21:** v1 created. Codex LLM-as-judge review (verdict GO-WITH-CHANGES) incorporated: renamed from `agentic-sdlc`, compressed to 5 stages, decision card emits composed pipelines not single modes, added supergit-trap and prepare-clean-mr as mandatory pre-flight, added Linear/GitLab CI/browser-verify routes. Gemini review deferred (quota exhausted; re-run scheduled). Security-incident sub-flow intentionally excluded by maintainer decision.
