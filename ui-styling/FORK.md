...
- No body changes to this skill's `SKILL.md`. **Bundles ~5.7MB of OFL fonts** under `canvas-fonts/` for canvas rendering. `scripts/shadcn_add.py` runs `npx shadcn@latest add` (network + exec).
...
`.claude/skills/ui-styling/` over this directory, then re-check this FORK.md.
